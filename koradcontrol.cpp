...
public:
    explicit Request(const std::string &query, size_t responseLen) 
        : query(query), responseLen(responseLen)
    {}
...
public:
    explicit IntRequest(const std::string &query, size_t responseLen, IntCB cb) 
        : Request(query, responseLen), cb(cb)
    {}
...
public:
    ...
    void open() {
        std::cerr << "Initiating KA3005P on port " << m_serialPortPath << "..." << std::endl;
        try {
            ...
            m_serialPort.set_option(as::serial_port_base::stop_bits(as::serial_port_base::stop_bits::one));
            m_serialPort.set_option(as::serial_port_base::parity(as::serial_port_base::parity::none));
            m_serialPort.set_option(as::serial_port_base::flow_control(as::serial_port_base::flow_control::none));
        } catch(const boost::system::system_error &e) {
            std::cerr << "Could not init serial port! Encountered error: " << e.what() << ". Terminating program." << std::endl;
            exit(EXIT_FAILURE);
        }
    }
    ...
    void startTimeout() {
        ...
        m_deadlineTimer.async_wait(std::bind(&KA3005P::stop, this, std::placeholders::_1));
    }
    ...
    void stop(const boost::system::error_code &ec) {
        ...
        std::cerr << "Timeout of " << TIMEOUT << " seconds expired! Terminating program." << std::endl;
        exit(EXIT_FAILURE);
    }

    void queryI(IntCB cb) {
        std::unique_ptr<IntRequest> req = std::make_unique<IntRequest>(REQUEST_SET_VOLTAGE, 5, cb);
        execRequestResponse(std::move(req));
    }

    void queryID(StrCB cb) {
        std::unique_ptr<StrRequest> req = std::make_unique<StrRequest>(REQUEST_ID, 16, cb);
        execRequestResponse(std::move(req));
    }

    void queryStatus(StrCB cb) {
        std::unique_ptr<StrRequest> req = std::make_unique<StrRequest>(REQUEST_STATUS, 1, cb);
        execRequestResponse(std::move(req));
    }

    void execRequestResponse(std::shared_ptr<Request> req) {
        if(m_serialPort.is_open()) m_serialPort.close();
        open();

        as::async_read(m_serialPort, as::mutable_buffer(m_readBuffer.data(), req->responseLen), [this, req](const boost::system::error_code &ec, size_t bytes) mutable {
            cancelTimeout();
            std::cerr << "Received Answer to " << req->query << std::endl;
            if(ec) { std::cerr << "Error during read! Error: " << ec.message() << ". Terminating program." << std::endl; exit(EXIT_FAILURE); }
            req->parseResponse(std::string(m_readBuffer.begin(), m_readBuffer.begin() + bytes));
            //flushBuffer();
        });

        startTimeout();
        //flushBuffer();
        as::async_write(m_serialPort, as::buffer(req->query), [this, req](const boost::system::error_code &ec, size_t bytes) mutable {
            cancelTimeout();
            if(ec) { std::cerr << "Error during write! Error: " << ec.message() << ". Terminating program." << std::endl; exit(EXIT_FAILURE); }
            ...
            if(req->responseLen == 0) {
                ...
                // Wait for 0.015s
                std::cerr << "Waiting then wait for answer." << req->query << std::endl;
...
public:
    ...
    int run(int argc, char* argv[]) {
        ...
        m_signals.async_wait(std::bind(&App::signalHandler, this, 
    ...
    void signalHandler(const boost::system::error_code &ec, int signalNumber) {
        if(ec) {
            std::cerr << "Error when catching signal! Error: " << ec.message() << std::endl;
            return;
        }
